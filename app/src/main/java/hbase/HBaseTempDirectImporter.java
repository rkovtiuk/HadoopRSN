 ...
 * efficient than {@link HBaseTempImporter} (since auto-flush is enabled) and
 * {@link HBaseTempBulkImporter} in particular.
 */
public class HBaseTempDirectImporter extends Configured implements Tool {
  ...
  static class HBaseTemperatureMapper<K, V> extends Mapper<LongWritable, Text, K, V> {
    ...
    public void map(LongWritable key, Text value, Context context) throws
      ...
      if (parser.isValidTemperature()) {
        ...
        p.add(HBaseTempQuery.DATA_COLUMNFAMILY,
            HBaseTempQuery.AIRTEMP_QUALIFIER,
  ...
  public int run(String[] args) throws Exception {
    if (args.length != 1) {
      System.err.println("Usage: HBaseTempDirectImporter <input>");
      return -1;
    }
  ...
  public static void main(String[] args) throws Exception {
    int exitCode = ToolRunner.run(HBaseConfiguration.create(),
        new HBaseTempDirectImporter(), args);
