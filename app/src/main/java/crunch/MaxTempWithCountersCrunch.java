...
// Crunch version of ch09-mr-features MaxTempWithCountersCrunch and MissingTemperatureFields
// Note that both are naturally combined into a single program.
public class MaxTempWithCountersCrunch {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage: MaxTempWithCountersCrunch <input path> <output path>");
            System.exit(-1);
        }

        Pipeline pipeline = new MRPipeline(MaxTempWithCountersCrunch.class);
